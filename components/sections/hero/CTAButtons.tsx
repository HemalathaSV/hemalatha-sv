  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3.5 sm:gap-4 w-full sm:w-auto">
      ...
      <div className="relative w-full sm:w-auto" ref={dropdownRef}>
        <div className="flex items-center w-full sm:w-auto rounded-full bg-[#0B1120] border border-slate-800 focus-within:ring-2 focus-within:ring-[#38BDF8] focus-within:border-transparent transition-all duration-200">
          <a
            href={PROFILE.resume}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => setIsOpen(false)}
            className="flex-1 sm:flex-none h-12 sm:h-[52px] pl-7 pr-4 sm:pl-8 sm:pr-5 inline-flex items-center justify-center rounded-l-full text-[#F8FAFC] font-medium text-sm sm:text-base tracking-wide hover:bg-[#111827] active:scale-[0.98] transition-all duration-200"
          >
            Resume
          </a>
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="h-12 sm:h-[52px] px-4 border-l border-slate-850 rounded-r-full text-[#94A3B8] hover:text-[#F8FAFC] hover:bg-[#111827] transition-all duration-200 flex items-center justify-center"
